...
def determine_severity(area, damage_type):
    """
    Determine the severity of the damage based on area and damage type.
    """
    if damage_type.lower() == 'dent':
        if area < 1000:
            return "Low Severity"
        elif area < 3000:
    ...
    elif damage_type.lower() == 'scratch':
        if area < 5000:
            return "Low Severity"
        elif area < 10000:
            return "Medium Severity"
...
def generate_dynamic_damage_report(template_path, output_path, data):
    ...
    for class_id, group in data.groupby('Class ID'):
        ...
        for idx, row in enumerate(group.iterrows(), start=1):
            area = row[1]['Area']
            severity = determine_severity(area, damage_type)
            bullet_text = f"    {damage_type} {idx}: {area} sq. mm - {severity}"
